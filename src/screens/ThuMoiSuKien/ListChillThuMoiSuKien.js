...
class ListChillThuMoiSuKien extends Component {
    constructor(props) {
        super(props);
    }
    _renderItemChild = ({ item, index }) => {
        return (
            <TouchableOpacity
                onPress={() => Utils.goscreen(this, 'sc_ThuMoiSuKien', { IDHocSinh: item.IDKhachHang, flagNotifiy: false, IDChiNhanh: item.IDChiNhanh })}
                style={{
                    flexDirection: "row",
                    paddingVertical: 8,
    ...
    render() {
        Utils.nlog('listChildThuMoiSuKienlistChildThuMoiSuKien', this.props.listChildThuMoiSuKien)
        return (
...
export default Utils.connectRedux(ListChillThuMoiSuKien, mapStateToProps, true);